...
function atualizarCalendario() {
    ...
    if (!calendario) return; // Proteção caso o elemento não exista
...
function criarDiaVazio() {
    const div = document.createElement('div');
    div.className = 'dia'; // Pode-se usar 'dia dia-vazio' se houver estilos específicos
    return div;
}
...
// Adicionamos 'DOMContentLoaded' para garantir que o HTML foi carregado antes de executar
document.addEventListener('DOMContentLoaded', () => {
    carregarConfiguracoesPersonalizadas();

    // Adicionar eventos de clique nos exemplos de fonte
    // (O código original tinha um erro aqui, pois 'fonte-exemplo' não existe no HTML fornecido)
    // Se esses elementos existirem, este código irá funcionar:
    ...
    // (O código original tinha um erro aqui, pois 'borda-exemplo' não existe no HTML fornecido)
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('/sw.js') // Assumindo que sw.js está na raiz
            .then(registration => {
